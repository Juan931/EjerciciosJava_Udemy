...
//Puntos que se van a guardar en un arreglo (point)
    
    private Point[] puntos = new Point[70];
    
    // La dimesion de la pantalla 
    ...
    protected void paintComponent(Graphics grphcs) {
        ...
        for (int i = 0; i < puntos.length; i++) {
            grphcs.drawOval(puntos[i].x, puntos[i].y, 10, 10);
            if (i < puntos.length -1) {
                grphcs.drawLine(puntos[i].x + 5, puntos[i].y + 5,
